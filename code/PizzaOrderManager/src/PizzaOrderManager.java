...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        Map<String, Integer> tmpMap = priceMap.newInstance();
        ...
        while (!read.atEOS()) {
            ...
            int bounds = line.indexOf(",");

            String k = line.substring(0, bounds);
            String v = line.substring(bounds + 1, line.length());
            Integer vInt = Integer.parseInt(v);

            tmpMap.add(k, vInt);
        }

        priceMap.transferFrom(tmpMap);
        read.close();
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        if (sizePriceMap.hasKey(size)) {
            sizePrice = sizePriceMap.value(size);
        }
        ...
        while (!line.equals("")) {
            if (toppingPriceMap.hasKey(line)) {
                toppingPrices += toppingPriceMap.value(line);
            }
            line = input.nextLine();
        }
        sizePrice = toppingPrices + sizePrice;
        return sizePrice;
    ...
    private static void putPrice(SimpleWriter output, int price) {
        ...
        priceCopy /= centsToDollars;
        String priceString = Double.toString(priceCopy);

        int decimal = priceString.indexOf(".");
        String dollars = priceString.substring(0, decimal);
        String cents = priceString.substring(decimal, priceString.length());
        cents = priceString.substring(0, 2); // only 2 decimal

        output.println("The price is $" + dollars + "." + cents);
