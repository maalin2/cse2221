...
public final class ArraysAndReferences {
    ...
    private static NaturalNumber productOfArrayElements(
        ...
        NaturalNumber lastTerm = new NaturalNumber2();

        // TODO - fill in body
        for (int i = 0; i < nnArray.length; i++) {
            lastTerm.copyFrom(nnArray[i]);
            product.multiply(lastTerm);
        }
    ...
    public static void main(String[] args) {
        ...
        NaturalNumber[] partialArray = partialProducts(array);
        for (int i = 0; i < partialArray.length; i++) {
            out.println(partialArray[i]);
        }
        out.println(product);
