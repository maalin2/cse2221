...
public final class CryptoUtilities {
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (pCopy.isZero()) {
            ...
            NaturalNumber nModM = n.divide(m);
            n.copyFrom(nModM);
            pCopy.multiply(new NaturalNumber2(2));
        } else {
            ...
            powerMod(nCopy, new NaturalNumber2(1), m);
            n.multiply(nCopy);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        if (wCopy.compareTo(new NaturalNumber2(1)) != 0) {
            isWitness = true;
        } else {
            wCopy.copyFrom(w);
            nCopy.decrement();
            ...
            isWitness = (wCopy.compareTo(new NaturalNumber2(1)) != 0);
        }
        return isWitness;
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            NaturalNumber2 nCopy = new NaturalNumber2(n);
            nCopy.decrement();
            final int numRandoms = 50;
            for (int i = 0; i < numRandoms && !isPrime; i++) {
                NaturalNumber random = randomNumber(nCopy);

                //@requires n > 2 and 1 < w < n - 1
                if ((random.compareTo(new NaturalNumber2(2)) < 0)
                ...
                if (isEven(random)) {
                    isPrime = false;
                } else if (isWitnessToCompositeness(random, nCopy)) {
                    isPrime = false;
                } else {
                    isPrime = true;
                }
